...
export const generationByIdReducer = (
  state = initialState,
  action: DefaultActionListType
): DefaultReducerObjectType => {
  switch (action.type) {
    case GET_GENERATIONS_POKEMON_LIST_START:
      return {
        ...state,
        loading: true,
        error: false,
      };
    case GET_GENERATIONS_POKEMON_LIST_SUCCESS:
      return {
        ...state,
        loading: false,
        data: action.payload || [],
      };
    case GET_GENERATIONS_POKEMON_LIST_FAIL:
      return {
        ...state,
        loading: false,
        error: true,
